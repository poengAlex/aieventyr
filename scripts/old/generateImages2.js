    ...
    messages: [
      {
        role: 'system',
        content:
          'You are an assistant specializing in generating consistent image styles for image generation.',
      },
...
async function generateCharacterDescription(cleanedText, imageArray) {
  ...
  if (!res.characters) {
    console.log('Error: No characters returned')
    console.log(res)
    return []
  }
...
async function generateImagePromt(character, imageStyle, story) {
  const prompt = `
    Create an image promt for an image of the charachter with the name: "${character.name}" based on the following character description.
    Description:
    ${character.description}


    Make sure to not change the Image style description, since this style should be used for all the images that we will generate using multiple promts.
    Consistent image style description:
    ${imageStyle}
    ...
    Only return the promt, no intro or outro.
    Make sure that the there is only an image of the character, and not multiple images of the same character. And make sure there is no text in the image
  `
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are an assistant specializing in creating good image promts for image generation using DaLL-E 3. Only return the promt, no intro or outro. Make sure that only one image of the character is generated and that there is no text in the image.',
      },
      { role: 'user', content: prompt },
    ],
    response_format: {
  ...
  try {
    ...
    await Promise.all(
      characters.map(async (character, index) => {
        ...
        const prompt = await generateImagePromt(character, imageStyle, cleanedText)
        // Save prompt to file
        const promptFilePath = path.join(outputDir, `${character.name}.txt`)
